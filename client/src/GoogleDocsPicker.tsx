...
function GoogleDocsPicker({ user, setUser, setGoogleDocs }) {
    ...
    async function listAddDoc(event) {
        try {
            ...
            setGoogleDocs(googleDocs => {
                const docAlreadyInList = googleDocs.some(doc => doc.google_id === docId);
                if (docAlreadyInList) return googleDocs.map(doc => (doc.google_id === docId) ? serverGoogleDoc : doc);
                return googleDocs.concat([serverGoogleDoc])
            });
        }
    ...
    if (!renderPicker) return (<div className="drive-button"><button onClick={async () => { await pick(); setRenderPicker(true); }}><strong>+ Choose from Google Drive</strong></button></div>);
    return (<>
        <div className="drive-button">
        <button onClick={pick}><strong>+ Choose from Google Drive</strong></button>
