...
export async function createUserDoc(userId: number, docId: number, revisionId: string, result: string = ""): Promise<any> {
    try {
        return await databaseQueryOnlyRow(pool, `INSERT INTO userdocs (user_id, doc_id, revision_id, result, analysis_start_time) VALUES ($1, $2, $3, $4, NOW()) RETURNING *;`, [userId, docId, revisionId, result]);
    }
    catch (error) {
        console.error(error);
        return null;
...
export async function updateRevisionIdTimeAndResult(userId: number, docId: number, revisionId: string, result: string | null = null) {
    const startNewAnalysis = (result === "");
    const analysisCompleted: boolean = (result !== null && result !== "");
    return await databaseQueryOnlyRow(pool, `UPDATE userdocs SET revision_id = $1, result = $2,
                                             analysis_start_time = CASE WHEN $3 THEN NOW() ELSE analysis_start_time END,
                                             last_analysis_time = CASE WHEN $4 THEN NOW() ELSE last_analysis_time END
                                             WHERE (user_id = $5 AND doc_id = $6) RETURNING *;`, [revisionId, result, startNewAnalysis, analysisCompleted, userId, docId]);
}

// NOTE: A result that is not null and not empty indicates that the contribution evaluation succeeded
export async function createOrUpdateUserDoc(userGoogleId: string, docGoogleId: string, revisionId: string, result: string | null = ""): Promise<any> {
    const user = await getUserByGoogleAccountId(userGoogleId);
    const doc = await getDocByGoogleId(docGoogleId);
    ...
    if (!userdoc) userdoc = await createUserDoc(user.id, doc.id, revisionId, result);
    else          userdoc = await updateRevisionIdTimeAndResult(user.id, doc.id, revisionId, result);
...
export async function getAllSubmittedByUser(userId: number): Promise<any> {
    return await databaseQueryRows(pool, `SELECT docs.id, docs.google_id, docs.name, revisions.modified_time, row_to_json(authors) as last_modifying_user FROM docs
